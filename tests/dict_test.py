...
class SqlAlchemyDictTypeTest(TestCase):
    def setUp(self):
        ...
        self.mytable = sa.Table(
            "mytable", metadata, sa.Column("name", sa.String), sa.Column("data", ObjectType)
        )
    ...
    def test_select_with_dict_column(self):
        ...
        self.assertSQL(
            "SELECT mytable.data['x'] AS anon_1 FROM mytable", select(mytable.c.data["x"])
        )
    ...
    def test_assign_to_object_type_after_commit(self):
        session, Character = self.set_up_character_and_cursor(return_value=[("Trillian", None)])
    ...
    def test_change_tracking(self):
        ...
        try:
            ...
            print(fake_cursor.fetchall.called)  # noqa: T201
            print(fake_cursor.mock_calls)  # noqa: T201
            raise
    ...
    def test_partial_dict_update_with_delitem(self):
        session, Character = self.set_up_character_and_cursor(return_value=[("Trillian", {"x": 1})])
